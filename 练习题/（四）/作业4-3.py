...
for i in range(len(x)):                                                        #每个列表循环一次（对应每行）
    for j in range(5):                                                         #每行循环5次（对应每列）
        ...
        x[i].append(float(h))                                                  #将元素添加进列表（矩阵）浮点型
        x[i].append(int(h))                                                    #将元素添加进列表（矩阵）整型
        #'''
...
for i in range(len(f)):                                                        #处理原矩阵A的数据
    f[i][i]=(f[i][i])/(i+1)                                             #斜率为负的对角线上的数除以行数
    if i != 2:                                                                 #跳过两条对角线的相交处，防止重复作除
        f[i][len(f)-1-i]=(f[i][len(f)-1-i])/(i+1)                       #斜率为正的对角线上的数除以行数
    print(f[i])                                                                #输出矩阵A2
